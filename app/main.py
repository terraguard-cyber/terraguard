...
PROXY_SECRET = os.environ["TERRAGUARD_PROXY_SECRET"]
MAX_LEN = 8000
...
def check_secret(request: Request):
    if request.headers.get("x-rapidapi-proxy-secret") != PROXY_SECRET:
...
async def analyze(body: HCLRequest, request: Request):
    ...
    if len(body.hcl) > MAX_LEN:
        raise HTTPException(status_code=413, detail=f"Input exceeds {MAX_LEN} characters")
    hcl = body.hcl
    ...
    try:
        ...
        log.error(f"analyze parse error raw={raw[:200]}")
...
async def secrets(body: HCLRequest, request: Request):
    ...
    try:
        ...
        log.error(f"secrets parse error raw={raw[:200]}")
        raise HTTPException(status_code=502, detail="Failed to parse analysis response")
